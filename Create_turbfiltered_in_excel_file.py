...
suffixe = '.csv'
print('day', type(day))
var = 'Turb_Turbfiltered'  # 'TSN2' or 'TSturb' or 'Turb_Turbfiltered'
...
if month == '06':
    if file_imer == 'CTD_VU_16-20_06.xlsx':
        transect = 'TJ4'  # TRANSECT TO CHANGE #T1 T2 T3 T4 or fixed
        # T1 : S1 à S11
        # T2 : S12 à S17
        # T3 : S18 S28
        # T4 : S29 S39
...
for s in sta:
    ...
    Turbidity2 = signal.medfilt(Turbidity)
    print('turb2', Turbidity2)
    Turbidity3 = signal.medfilt(Turbidity, 5)
    ...
    print('len turbidity', len(Turbidity))

    #file_excel='CTD_imer_aug2022_copy.xlsx'
    #file_excel='test.xlsx'
    file_excel='/home/penicaud/Documents/Data/CTD/'+survey+'/'+survey+'_for_surfer_turbfiltered.xlsx' #survey_august/TA_for_surfer_copy.xlsx'
    ...
    if index==1:
        data_Turbidity2.to_excel(writer, sheet_name=transect, startrow=0, startcol=6, index=False, header=True)#, index_label='Turbidity Median Filter 3')
        data_Turbidity3.to_excel(writer, sheet_name=transect, startrow=0, startcol=7, index=False, header=True)#, index_label='Turbidity Median Filter 3')
...
"""
    ...
    if var == 'TSturb' or var == 'TSN2':

        fig, axs = plt.subplots(
            ncols=3)  # , ncols=2)#fonctionne avec plot:share=1,left=3 , right=5,bottom=5,top=7,wspace=10, hspace=5)
        ...
        t2 = ax.plot(T_imer, -depth_imer, color='gold', label=hour_imer + 'h' + mn_imer)
        ...
        s2 = ax.plot(S_imer, -depth_imer, color='lightgreen', label=hour_imer + 'h' + mn_imer)
        ...
        if var == 'TSturb':
            ...
            # s1= ax.plot(S_LOG, -depth_LOG, color='blue', label='CTD LOG '+hour_LOG+'h'+mn_LOG )
            # s2 = ax.scatter(S_imer, -depth_imer, alpha=0.8, marker='x', color='lightgreen', label='CTD IMER')
            s2 = ax.plot(Turbidity, -depth_imer, color='brown', label=hour_imer + 'h' + mn_imer)
            # color=color[i] , label=hour+'h'+mn)#, label='GFF')
            ax.set_xlabel('Salinity (PSU)')  # ('Absolute Salinity (g/kg)', fontsize=fontsize)
            # ax.set_ylabel('Depth (m)', fontsize=fontsize)
    ...
    elif var == 'Turb_Turbfiltered':
        fig, axs = plt.subplots(
            ncols=4)  # , ncols=2)#fonctionne avec plot:share=1,left=3 , right=5,bottom=5,top=7,wspace=10, hspace=5)
        fig.suptitle(day + '/' + month + '/' + year + '. ' + transect + ', ' + station)

        ax = axs[0]
        ax.set_xlim(0, 200)  # (25,31)
        ax.set_ylim(-18, 0)
        # t1 = ax.scatter(T_LOG, -depth_LOG, alpha=0.8, marker='x', color='red', label='CTD LOG')
        # t1=ax.plot(T_LOG, -depth_LOG, color='red', label='CTD LOG '+hour_LOG+'h'+mn_LOG )  # color=color[i], label=hour+'h'+mn)
        # t2 = ax.scatter(T_imer, -depth_imer, alpha=0.8, marker='x', color='gold', label='CTD IMER')
        t2 = ax.plot(Turbidity, -depth_imer, color='brown', label='raw data')
        ax.set_xlabel('Temperature (°C)', fontsize=fontsize)  # ('Conservative Temperature (°C)', fontsize=fontsize)
        ax.set_ylabel('Depth (m)', fontsize=fontsize)
        ax.xaxis.set_major_locator(MultipleLocator(50))
        ax.xaxis.set_major_formatter(FormatStrFormatter('%d'))
        ...
        # s1= ax.plot(S_LOG, -depth_LOG, color='blue', label='CTD LOG '+hour_LOG+'h'+mn_LOG )
        # s2 = ax.scatter(S_imer, -depth_imer, alpha=0.8, marker='x', color='lightgreen', label='CTD IMER')
        s2 = ax.plot(Turbidity2, -depth_imer, color='brown', label='median filter 3')
        # color=color[i] , label=hour+'h'+mn)#, label='GFF')
        ax.set_xlabel('Turbidity filtered')  # ('Absolute Salinity (g/kg)', fontsize=fontsize)
        # ax.set_ylabel('Depth (m)', fontsize=fontsize)
        ax.xaxis.set_major_locator(MultipleLocator(50))
        ax.xaxis.set_major_formatter(FormatStrFormatter('%d'))
        ax.xaxis.set_minor_locator(MultipleLocator(10))
        ...
        # s1= ax.plot(S_LOG, -depth_LOG, color='blue', label='CTD LOG '+hour_LOG+'h'+mn_LOG )
        # s2 = ax.scatter(S_imer, -depth_imer, alpha=0.8, marker='x', color='lightgreen', label='CTD IMER')
        s2 = ax.plot(Turbidity3, -depth_imer, color='brown', label='median filter 5')
        # color=color[i] , label=hour+'h'+mn)#, label='GFF')
        ax.set_xlabel('Turbidity filtered')  # ('Absolute Salinity (g/kg)', fontsize=fontsize)
        # ax.set_ylabel('Depth (m)', fontsize=fontsize)
        ax.xaxis.set_major_locator(MultipleLocator(50))
        ...
        # s1= ax.plot(S_LOG, -depth_LOG, color='blue', label='CTD LOG '+hour_LOG+'h'+mn_LOG )
        # s2 = ax.scatter(S_imer, -depth_imer, alpha=0.8, marker='x', color='lightgreen', label='CTD IMER')
        s2 = ax.plot(Turbidity - Turbidity2, -depth_imer, color='black', label='median filter 3')
        s3 = ax.plot(Turbidity - Turbidity3, -depth_imer, color='red', label='median filter 3')
        ax.axvline(-lim / 2, ls='dashed', color='grey')
        ax.axvline(lim / 2, ls='dashed', color='grey')
        # color=color[i] , label=hour+'h'+mn)#, label='GFF')
        ax.set_xlabel('residual')  # ('Absolute Salinity (g/kg)', fontsize=fontsize)
        # ax.set_ylabel('Depth (m)', fontsize=fontsize)
        ax.xaxis.set_major_locator(MultipleLocator(10))
        ax.xaxis.set_major_formatter(FormatStrFormatter('%d'))
        ax.xaxis.set_minor_locator(MultipleLocator(5))
    ...
    outfile = var + day + month + year + '_' + station + '.png'
